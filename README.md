...
**從旅客的一句提問，到查得到依據的下一班列車。**

這是一個結合 **自行整理資料、Qwen 模型微調與網頁展示**的個人實作專案。使用者可以在路線圖選站，也可以用中文提問，查詢機場捷運的預定發車時間。

專案已在 **RTX 4060 8 GB** 上完成兩輪 LoRA 訓練實驗，並將第二輪訓練結果接入本機展示介面。實作涵蓋資料整理、模型微調、班表核對與前端操作，定位為可在本機重現的 AI 應用作品，非桃園捷運官方服務。
...
*實際操作截圖：在 Qwen 模式輸入「A12 到 A18 末班車幾點」，介面顯示 2026-09-22 營運日的末班車為 **23:58 普通車**，並標示可到達 A18 高鐵桃園、不需換車。結果依本專案的一般班表產生。*
